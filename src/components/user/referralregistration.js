...
import {CREATE_USER, ADD_REFERRAL, ADD_REFERRAL_CLICK} from '../graphql/MutationResolver';
...
import { instanceOf } from 'prop-types';
  ...
  class ReferralRegistration extends Component{
    static propTypes = {
    ...
    constructor(props) {
      ...
      <h6 style={{color: 'rebeccapurple'}}>Referred to Zimlancer by {this.props.match.params.referredby}</h6>
      <br/>
        <Row>
          <Col>
    ...
    componentDidMount() {
      const { cookies } = this.props;
        ...
        axios.post(GRAPHQL_BASE_URL, {/////add referral click
          query: print(ADD_REFERRAL_CLICK), variables: {
  userid: cookies.get('userId'),
  referredby: this.props.match.params.referredby,
  date: date//////////date without time
        }
      }).then((result) => {
  this.setState({ref2: result.data.data.addReferralClicks})
      }).catch(error => {
...
handleSubmit(e) {
  ...
  this.setState({userDetails: result.data.data.createUser});
    ...
    axios.post(GRAPHQL_BASE_URL, {
        query: print(ADD_REFERRAL), variables: {
userid: result.data.data.createUser.id,
referredby: this.props.match.params.referredby,
date: dateTime
      }
    }).then((result) => {
this.setState({ref: result.data.data.addReferral})
    }).catch(error => {
      console.log(error.response);      
    });
    let port = (window.location.port ? ':' + window.location.port : '');
  ...
  export default withCookies(ReferralRegistration);