...
<h6 style={{color: 'rebeccapurple'}}><br/><br/>You need to login first inorder to contact this seller.<br/><br/></h6>
</Modal>
}else{
...
var dateTime = date+' '+time;
  ...
  handleChange(e) {
    ...
    render() {
      const { open } = this.state;
        ...
        <button onClick={this.onOpenModal} style={{backgroundColor: '#fff', border: '1px solid #fff'}}><h6 style={{}}><FaRegEnvelope color="rebeccapurple" size="20px"/> Inbox seller</h6></button>
        <this.ValidateSignIn/>
        </div>
        )
    }
  ...
  export default withCookies(MessagePopup);